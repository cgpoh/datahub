...
public class OpenLineageEventToDatahubTest extends TestCase {
  ...
  public void testGenerateUrnFromStreamingDescriptionWithStreamingSpec()
            ...
            new HashMap<>() {
              {
                ...
                put(
                    SparkConfigParser.STREAMING_PLATFORM_KEY
                        + ".iceberg.stream1."
                        + SparkConfigParser.STREAMING_IO_PLATFORM_TYPE,
                    "sink");
                put(
                    SparkConfigParser.STREAMING_PLATFORM_KEY
                        + ".iceberg.stream1.usePlatformInstance",
                    true);
                put(
                    SparkConfigParser.STREAMING_PLATFORM_KEY
                        + ".iceberg.stream1."
                        + SparkConfigParser.PLATFORM_INSTANCE_KEY,
                    "test-dev");
    ...
    Optional<DatasetUrn> urn =
        SparkStreamingEventToDatahub.generateUrnFromStreamingDescription(
            "stream2.tmp_namespace.streaming_source", sparkLineageConfBuilder.build());
    assert (urn.isPresent());

    String result = urn.get().toString();
    Assert.assertEquals(
        "urn:li:dataset:(urn:li:dataPlatform:iceberg,stream2.tmp_namespace.streaming_source,DEV)",
        result);

    urn =
        SparkStreamingEventToDatahub.generateUrnFromStreamingDescription(
            "stream1.tmp_namespace_1.streaming_sink", sparkLineageConfBuilder.build(), true);
    assert (urn.isPresent());

    result = urn.get().toString();
    Assert.assertEquals(
        "urn:li:dataset:(urn:li:dataPlatform:iceberg,test-dev.tmp_namespace_1.streaming_sink,PROD)",
        result);

    urn =
        SparkStreamingEventToDatahub.generateUrnFromStreamingDescription(
            "stream1.tmp_namespace_1.streaming_sink", sparkLineageConfBuilder.build());
    assert (urn.isEmpty());

    urn =
        SparkStreamingEventToDatahub.generateUrnFromStreamingDescription(
            "stream2.tmp_namespace_1.streaming_source", sparkLineageConfBuilder.build(), true);
    assert (urn.isEmpty());
  }
